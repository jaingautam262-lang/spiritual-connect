...
import { LO_SHU_GRID_LAYOUT, LO_SHU_INTERPRETATIONS } from '../data/numerologyInterpretations';

export default function BusinessLoShuGrid() {
  const [name, setName] = useState('');
  const [result, setResult] = useState<{ grid: number[][]; present: number[]; missing: number[] } | null>(null);
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start justify-center">
              ...
              <div>
                <div className="grid grid-cols-3 gap-2 w-48 mx-auto">
                  {LO_SHU_GRID_LAYOUT.flat().map((num) => {
                    ...
                    const count = result.grid[Math.floor(LO_SHU_GRID_LAYOUT.flat().indexOf(num) / 3)][LO_SHU_GRID_LAYOUT.flat().indexOf(num) % 3];
                    return (
                      <div
                        key={num}
                        className="h-14 w-14 rounded-xl flex flex-col items-center justify-center border-2 transition-all"
                        style={{
                          borderColor: isPresent ? 'oklch(0.68 0.20 48)' : 'oklch(0.78 0.14 75 / 0.2)',
                          background: isPresent
                            ? 'linear-gradient(135deg, oklch(0.68 0.20 48 / 0.15), oklch(0.78 0.14 75 / 0.1))'
                            : 'oklch(0.94 0.025 80)',
                        }}
                      >
                        <span className="font-decorative font-bold text-xl leading-none" style={{ color: isPresent ? 'oklch(0.68 0.20 48)' : 'oklch(0.70 0.04 60)' }}>
                          {num}
                        </span>
                        {count > 0 && (
                          <span className="text-xs font-heading" style={{ color: 'oklch(0.68 0.20 48)' }}>×{count}</span>
                        )}
                      </div>
                    );
                  })}
              ...
              <div className="flex-1 space-y-2 max-w-sm">
                {result.present.length > 0 && (
                  <div>
                    <p className="font-heading font-semibold text-xs mb-2" style={{ color: 'oklch(0.45 0.14 140)' }}>
                      Strengths (Present Numbers)
                    </p>
                    {result.present.map((n) => (
                      <div key={n} className="text-xs font-body p-2 rounded-lg mb-1" style={{ background: 'oklch(0.65 0.16 140 / 0.06)', border: '1px solid oklch(0.65 0.16 140 / 0.15)' }}>
                        <span className="font-heading font-bold" style={{ color: 'oklch(0.45 0.14 140)' }}>{n}: </span>
                ...
                {result.missing.length > 0 && (
                  <div>
                    <p className="font-heading font-semibold text-xs mb-2" style={{ color: 'oklch(0.55 0.22 25)' }}>
                      Missing Numbers — Remedies
                    </p>
                    {result.missing.map((n) => (
                      <div key={n} className="text-xs font-body p-2 rounded-lg mb-1" style={{ background: 'oklch(0.55 0.22 25 / 0.06)', border: '1px solid oklch(0.55 0.22 25 / 0.15)' }}>
                        <span className="font-heading font-bold" style={{ color: 'oklch(0.55 0.22 25)' }}>{n}: </span>
