...
export default function CompanyNameGenerator() {
  const [form, setForm] = useState({ ownerDob: '', purpose: 'Wealth', keyword: '' });
  const [results, setResults] = useState<Array<{ name: string; number: number }> | null>(null);
      ...
      <div className="ornamental-border rounded-2xl p-6 bg-card mb-6">
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-heading font-semibold mb-1" style={{ color: 'oklch(0.35 0.12 25)' }}>
                Owner Date of Birth
              </label>
              <input
                type="date"
            ...
            >
              <div>
                ...
                <p className="text-xs font-body text-muted-foreground">
                  {FAVORABLE.includes(r.number) ? '✅ Favorable number' : '⚠️ Consider correction'}
                </p>
              </div>
              <div
                className="h-12 w-12 rounded-full flex items-center justify-center font-decorative font-bold text-xl"
                style={{
                  background: FAVORABLE.includes(r.number) ? 'oklch(0.65 0.16 140 / 0.15)' : 'oklch(0.78 0.14 75 / 0.15)',
                  color: FAVORABLE.includes(r.number) ? 'oklch(0.45 0.14 140)' : 'oklch(0.55 0.16 60)',
                }}
              >
                {r.number}
              </div>
