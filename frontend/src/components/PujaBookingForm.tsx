...
import { useGetAllTemples, useCreatePujaBooking } from '../hooks/useQueries';
import { toast } from 'sonner';
import { CheckCircle } from 'lucide-react';

const PUJA_TYPES = [
  'Ganesh Puja', 'Lakshmi Puja', 'Shiva Abhishek', 'Satyanarayan Katha',
  'Navgraha Puja', 'Rudrabhishek', 'Sunderkand Path', 'Hanuman Puja',
  'Durga Puja', 'Kali Puja', 'Saraswati Puja', 'Vishnu Puja',
  'Ram Puja', 'Krishna Puja', 'Vastu Puja', 'Griha Pravesh Puja',
];
...
export default function PujaBookingForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.templeId || !form.devoteeName || !form.pujaType || !form.preferredDate) {
  ...
  if (confirmed) {
    return (
      <div className="max-w-lg mx-auto text-center py-12">
        <CheckCircle className="h-16 w-16 mx-auto mb-4" style={{ color: 'oklch(0.65 0.16 140)' }} />
        <h2 className="font-heading text-2xl font-bold mb-2" style={{ color: 'oklch(0.35 0.12 25)' }}>
          Puja Booked Successfully!
        </h2>
        <p className="font-body text-muted-foreground mb-4">
          Your booking ID: <span className="font-heading font-bold" style={{ color: 'oklch(0.68 0.20 48)' }}>{confirmed.slice(0, 20)}...</span>
        </p>
        <p className="font-body text-sm text-muted-foreground mb-6">
          You will receive confirmation once the temple processes your booking. Track it in your Dashboard.
        </p>
        <button
          onClick={() => { setConfirmed(null); setForm({ templeId: '', devoteeName: '', gotra: '', pujaType: '', preferredDate: '', specialWishes: '' }); }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                {PUJA_TYPES.map((p) => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-heading font-semibold mb-1" style={{ color: 'oklch(0.35 0.12 25)' }}>
