...
export default function Reports() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!identity) { toast.error('Please login to request a report'); return; }
    if (!form.name || !form.dob) { toast.error('Please fill required fields'); return; }
  ...
  return (
    <div>
      <div className="relative w-full overflow-hidden" style={{ minHeight: '280px' }}>
        <img src="/assets/generated/panchang-banner.dim_1200x400.png" alt="Reports" className="w-full h-72 object-cover" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {REPORT_CATALOG.filter((r) => r.category === cat).map((r) => (
                <div key={r.id} className="temple-card p-5 flex flex-col">
                  ...
                  <h3 className="font-heading font-bold text-sm mb-2" style={{ color: 'oklch(0.22 0.08 22)' }}>{r.name}</h3>
                  <p className="font-body text-xs text-muted-foreground mb-3 flex-1">{r.description}</p>
                  <div className="flex items-center justify-between mb-3">
                    <span className="font-heading font-bold" style={{ color: 'oklch(0.68 0.20 48)' }}>₹{r.price}</span>
                    <span className="text-xs font-body text-muted-foreground">{r.deliveryDays} day{r.deliveryDays > 1 ? 's' : ''}</span>
                  </div>
                  <button
                    onClick={() => { setSelectedReport(r.id); setConfirmed(false); }}
      ...
      {selectedReport && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-2xl p-6 shadow-2xl" style={{ background: 'oklch(0.97 0.015 85)', border: '1px solid oklch(0.78 0.14 75 / 0.3)' }}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-heading font-bold text-lg" style={{ color: 'oklch(0.35 0.12 25)' }}>
                {confirmed ? 'Report Requested!' : `Request: ${report?.name}`}
              </h2>
              <button onClick={() => { setSelectedReport(null); setConfirmed(false); }} className="p-1 rounded-full hover:bg-muted">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            {confirmed ? (
              <div className="text-center py-6">
                <CheckCircle className="h-12 w-12 mx-auto mb-3" style={{ color: 'oklch(0.65 0.16 140)' }} />
                <p className="font-body text-sm text-muted-foreground">Your report has been requested. Our astrologers will prepare it within {report?.deliveryDays} day(s). Check your Dashboard for updates.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-3">
                {!identity && <div className="p-3 rounded-lg text-sm font-body text-center" style={{ background: 'oklch(0.78 0.14 75 / 0.1)', color: 'oklch(0.55 0.16 60)' }}>Please login to request a report</div>}
                <div>
                  ...
                  <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                    placeholder="Your full name" className="w-full px-3 py-2 rounded-lg border text-sm font-body focus:outline-none bg-background"
                ...
                <div>
                  ...
                  <input type="text" value={form.pob} onChange={(e) => setForm({ ...form, pob: e.target.value })}
                    placeholder="City, Country" className="w-full px-3 py-2 rounded-lg border text-sm font-body focus:outline-none bg-background"
                    style={{ borderColor: 'oklch(0.78 0.14 75 / 0.3)' }} />
                </div>
                <div className="flex items-center justify-between pt-1">
                  <span className="font-heading font-bold" style={{ color: 'oklch(0.68 0.20 48)' }}>₹{report?.price}</span>
                  <button type="submit" disabled={createReport.isPending || !identity}
                    className="px-6 py-2 rounded-full font-heading font-bold text-sm transition-all disabled:opacity-50"
                    style={{ background: 'linear-gradient(135deg, oklch(0.68 0.20 48), oklch(0.58 0.18 40))', color: 'white' }}>
                    {createReport.isPending ? 'Submitting...' : 'Request Report'}
                  </button>
                </div>
              </form>
            )}
