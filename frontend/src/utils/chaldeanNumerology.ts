...
export function extractLoShuNumbers(dob: string): number[] {
  return dob.replace(/[^0-9]/g, '').split('').map(Number).filter((n) => n >= 1 && n <= 9);
}
...
export function calculateCompatibility(name1: string, dob1: string, name2: string, dob2: string): number {
...
export function generateNameSuggestions(baseName: string, targetNumbers: number[] = [1, 5, 6, 9]): Array<{ name: string; number: number }> {
  ...
  for (const suffix of suffixes) {
    ...
    if (targetNumbers.includes(num) && !suggestions.find((s) => s.name === candidate)) {
