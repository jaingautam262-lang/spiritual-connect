...
export default function BhajanLibrary() {
  ...
  const bhajans =
    backendBhajans && backendBhajans.length > 0 ? backendBhajans : MOCK_BHAJANS;
      ...
      <div className="relative h-48 md:h-64 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 to-black/70 flex flex-col items-center justify-center text-center px-4">
          ...
          <p className="text-amber-200 text-sm md:text-base">
            Divine melodies for the soul — play, listen, and immerse
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {isListening && (
          <p className="text-sm text-primary mb-4 animate-pulse">
            🎙️ Listening... speak the bhajan name or deity
          </p>
        )}
              ...
              {filtered.map((bhajan, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p
                      className={`font-medium text-sm truncate ${idx === currentIndex ? "text-primary" : "text-foreground"}`}
                    >
                      {bhajan.title}
                    </p>
                    <p className="text-xs text-muted-foreground truncate">
                      {bhajan.artist} • {bhajan.deity}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    >
                      <Heart
                        className="w-4 h-4"
                        fill={
                          favorites.has(bhajan.id) ? "currentColor" : "none"
                        }
                      />
                    </button>
                  </div>
