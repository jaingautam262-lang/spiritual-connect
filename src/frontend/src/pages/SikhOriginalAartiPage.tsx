...
import { sikhOriginalAartis } from "../data/sikhOriginalAartis";
...
function SikhAartiCard({
  aarti,
}: { aarti: (typeof sikhOriginalAartis)[number] }) {
  const [expanded, setExpanded] = useState(false);
  const [lang, setLang] = useState<"gurmukhi" | "roman" | "hindi" | "english">(
    "gurmukhi",
  );

  const displayText = (() => {
    if (lang === "gurmukhi") return aarti.gurmukhi;
    if (lang === "roman") return aarti.transliteration;
  ...
  return (
    <div
      ...
      data-ocid={`sikh_aarti.card.${aarti.id}`}
    >
      {/* Header */}
      <div
      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          <Badge
            style={{
              background: "oklch(0.58 0.18 280 / 0.2)",
              color: "oklch(0.78 0.10 280)",
              border: "1px solid oklch(0.58 0.18 280 / 0.3)",
            }}
          >
            Gurmukhi
          </Badge>
      ...
      <div className="flex border-b" style={{ borderColor: BORDER }}>
        {(["gurmukhi", "roman", "hindi", "english"] as const).map((tab) => (
          <button
            key={tab}
            type="button"
            onClick={() => setLang(tab)}
            ...
            data-ocid={`sikh_aarti.lang_tab.${tab}`}
          >
            {tab === "gurmukhi"
              ? "ਗੁਰਮੁਖੀ"
              : tab === "roman"
      ...
      <div className="p-5">
        ...
        >
          <pre
            className="whitespace-pre-wrap text-sm font-body leading-relaxed"
            style={{
              color:
                lang === "gurmukhi"
                  ? "oklch(0.90 0.06 75)"
              ...
              fontFamily:
                lang === "gurmukhi"
                  ? "'Noto Sans Gurmukhi', 'Noto Serif Gurmukhi', serif"
                  : "inherit",
              lineHeight: "2",
        ...
        <button
          ...
          data-ocid={`sikh_aarti.expand_button.${aarti.id}`}
        >
...
export default function SikhOriginalAartiPage() {
  const [search, setSearch] = useState("");

  const filtered = sikhOriginalAartis.filter(
    (a) =>
      !search ||
      a.name.toLowerCase().includes(search.toLowerCase()) ||
      (a.ragAndMahala ?? "").toLowerCase().includes(search.toLowerCase()),
  );
    ...
    <div className="min-h-screen" style={{ background: "oklch(0.14 0.05 20)" }}>
      ...
      >
        <div className="container mx-auto max-w-4xl text-center">
          <div className="text-4xl mb-3">ੴ</div>
          <h1
          ...
          >
            Sikh Aartis in Original Gurmukhi Script
          </p>
          <p
            className="text-sm max-w-2xl mx-auto mb-8"
            style={{ color: MUTED }}
          >
            Sacred Sikh devotional compositions from Guru Granth Sahib Ji — in
            original Gurmukhi script with Roman transliteration, Hindi &amp;
            English meaning.
          </p>
          ...
          <div className="relative max-w-md mx-auto">
            ...
            <Input
              ...
              placeholder="Search by name or Raag/Mahala..."
              className="pl-10"
              style={{
              ...
              data-ocid="sikh_aarti.search_input"
            />
          </div>
        </div>
      </div>
      ...
      <div
        className="py-4 px-4"
        style={{
          background: "oklch(0.58 0.18 280 / 0.10)",
          borderBottom: "1px solid oklch(0.58 0.18 280 / 0.18)",
        }}
      >
        <div className="container mx-auto max-w-4xl">
          ...
          >
            ੴ ਸਤਿ ਨਾਮੁ — ਇਹ ਆਰਤੀਆਂ ਸ੍ਰੀ ਗੁਰੂ ਗ੍ਰੰਥ ਸਾਹਿਬ ਜੀ ਤੋਂ ਲਈਆਂ ਗਈਆਂ ਹਨ। All hymns are
            from Guru Granth Sahib Ji — the eternal Guru of the Sikhs.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto max-w-4xl px-4 py-10">
        {filtered.length === 0 ? (
          <div className="text-center py-20" data-ocid="sikh_aarti.empty_state">
          ...
          <div
            ...
            data-ocid="sikh_aarti.list"
          >
            {filtered.map((aarti) => (
              <SikhAartiCard key={aarti.id} aarti={aarti} />
            ))}
          </div>
        )}
