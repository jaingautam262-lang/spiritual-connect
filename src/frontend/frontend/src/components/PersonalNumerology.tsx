...
export default function PersonalNumerology() {
  ...
  const getInterp = (num: number) =>
    NUMBER_INTERPRETATIONS.find((n) => n.number === num);
      ...
      {result && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              {
                label: "Moolank (Driver)",
                value: result.moolank,
                desc: "Life Path Number",
              },
              {
                label: "Bhagyank (Destiny)",
              ...
              return (
                <div
                  key={item.label}
                  className="ornamental-border rounded-xl p-5 bg-card text-center"
                >
                  <p className="font-heading text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2">
                    {item.label}
                  </p>
                  <div
                    className="text-5xl font-decorative font-bold mb-2"
                  ...
                  >
                    {interp?.description.slice(0, 100)}...
                  </p>
                </div>
              );
            })}
