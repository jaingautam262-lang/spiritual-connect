...
export default function PrasadDeliveryForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!identity) {
      toast.error("Please login to request prasad delivery");
      return;
    }
    if (!form.templeId || !form.address || !form.mobileNumber) {
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-12">
        ...
        <p className="font-body text-muted-foreground mb-6">
          Sacred prasad will be delivered to your doorstep within 7-10 business
          days.
        </p>
        <button
          type="button"
          onClick={() => {
            setConfirmed(false);
            setForm({ templeId: "", address: "", mobileNumber: "" });
          }}
